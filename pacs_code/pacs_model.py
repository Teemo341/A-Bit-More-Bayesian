...
class BayesLinear_Normalq(nn.Module):
    ...
    def __init__(self, n_in, n_out, prior_class, with_bias=True):
        ...
        self.W_mu = nn.Parameter(torch.Tensor(
            self.n_in, self.n_out).uniform_(-0.1, 0.1))
        # self.W_mu = nn.Parameter(torch.Tensor(self.n_in, self.n_out).normal_(-0.1, 0.1))
        self.W_p = nn.Parameter(torch.Tensor(
            self.n_in, self.n_out).uniform_(-3, -2))
        ...
        if not ifsample:  # When training return MLE of w for quick validation
            ...
            if self.with_bias:
                output = torch.mm(X, self.W_mu) + \
                    self.b_mu.expand(X.size()[0], self.n_out)
            else:
                output = torch.mm(X, self.W_mu)
            ...
            if not local_rep:
                ...
                if self.with_bias:
                    lqw = isotropic_gauss_loglike(
                        W, W_mu, std_w) + isotropic_gauss_loglike(b, b_mu, std_b)
                ...
                if self.with_bias:
                    # (batch_size, n_output)
                    output = torch.mm(X, W) + \
                        b.unsqueeze(0).expand(X.shape[0], -1)
                else:
                    output = torch.mm(X, W)
                ...
                if self.with_bias:
                    # (batch_size, n_output)
                    output = torch.bmm(
                        X.view(X.size()[0], 1, X.size()[1]), W).squeeze() + b
                    lqw = isotropic_gauss_loglike(
                        W, W_mu, std_w) + isotropic_gauss_loglike(b, b_mu, std_b)
                    lpw = self.prior.loglike(W) + self.prior.loglike(b)
                else:
                    output = torch.bmm(
                        X.view(X.size()[0], 1, X.size()[1]), W).squeeze()
                    lqw = isotropic_gauss_loglike(W, W_mu, std_w)
                    lpw = self.prior.loglike(W)
...
class net0(nn.Module):
    def __init__(self, num_class, mctimes, prior_type, with_bias=True, local_rep=True, norm=False, fe='no', num_bfe=1):
        ...
        if self.feature_extractor == 'linear':
            ...
            if self.num_bfe == 2:
                self.bayesian_layer0 = BayesLinear_Normalq(
                    512, 512, isotropic_mixture_gauss_prior(), with_bias=True)
            self.bayesian_layer = BayesLinear_Normalq(
                512, 512, isotropic_mixture_gauss_prior(), with_bias=True)
        ...
        if self.prior_type == 'NO':
            ...
            self.bayesian_classfier = BayesLinear_Normalq(
                512, self.num_class, isotropic_mixture_gauss_prior(), with_bias=self.with_bias)
    ...
    def forward(self, x, label, meta_im, meta_l, meta_classes, num_domains, num_samples_perclass, withnoise=True, hierar=False, sampling=True):
        ...
        if self.feature_extractor == 'bayes' and self.num_bfe == 2 and sampling:
            z_mu = torch.mm(z, self.bayesian_layer0.W_mu) + \
                self.bayesian_layer0.b_mu.expand(z.size()[0], 512)
            mf_mu = torch.mm(meta_fea, self.bayesian_layer0.W_mu) + \
                self.bayesian_layer0.b_mu.expand(meta_fea.size()[0], 512)
            ...
            if self.training:
                ...
                kl20 = 0
            all_fe, phi_entropy0 = self.bayesian_layer0(
                torch.cat([z, meta_fea], 0), self.MCtimes, self.local_rep, ifsample=sampling)
            ...
            z = all_fe[:z.size()[0]].view(z.size()[0], feature_samples, 512).view(
                z.size()[0]*feature_samples, 512)
            ...
            feature_samples = 1
            all_fe, phi_entropy0 = self.bayesian_layer0(
                torch.cat([z, meta_fea], 0), self.MCtimes, self.local_rep, sampling)
        ...
        if self.feature_extractor == 'linear' and self.training:
            ...
            kl2 = self.domain_invariance_kl(z_mu[:, 0, :], std_z[:, 0, :],
                                            label, mf_mu[:, 0, :].view(
                                                self.num_class, num_domains*num_samples_perclass, -1),
            ...
            z_mu = torch.mm(z, self.bayesian_layer.W_mu) + \
                self.bayesian_layer.b_mu.expand(z.size()[0], 512)
            mf_mu = torch.mm(meta_fea, self.bayesian_layer.W_mu) + \
                self.bayesian_layer.b_mu.expand(meta_fea.size()[0], 512)
            ...
            if self.training:
                ...
                kl2 = kl20 + self.domain_invariance_kl(z_mu.view(-1, self.MCtimes**(self.num_bfe-1), 512), std_z.view(-1, self.MCtimes**(self.num_bfe-1), 512), label,
                                                       mf_mu.view(
                                                           meta_classes, num_domains*num_samples_perclass, self.MCtimes**(self.num_bfe-1), -1),
                                                       std_mf.view(meta_classes, num_domains*num_samples_perclass, self.MCtimes**(self.num_bfe-1), -1))

            else:
                kl2 = kl20 + 0
            all_fe, phi_entropy = self.bayesian_layer(
                torch.cat([z, meta_fea], 0), self.MCtimes, self.local_rep, ifsample=sampling)
        ...
        if self.prior_type == 'SGP':
            ...
            # pdb.set_trace()
            all_f, theta_entropy = self.bayesian_classfier(
                torch.cat([z, meta_fea0], 0), self.MCtimes, self.local_rep, sampling)
            ...
            if not self.local_rep and sampling:
                ...
                # whether 2 or 1 layers
                y0 = y00.view(x.size()[0], feature_samples **
                              self.num_bfe, self.MCtimes, self.num_class)
            ...
            y = self.classifier(z)
            y0 = y.view(x.size()[0], feature_samples **
                        self.num_bfe, 1, self.num_class)

            meta_fea0 = meta_fea.view(-1, meta_fea.size()[-1])
            y_meta = self.classifier(meta_fea0)
            y_meta = y_meta.view(meta_classes, num_domains*num_samples_perclass,
                                 feature_samples ** self.num_bfe, 1, self.num_class)
    ...
    def domain_invariance_kl(self, x_m, x_s, label, mx_m, mx_s):
        ...
        kld = 0.5*(torch.log(1e-6 + mx_s0) - torch.log(1e-6 + x_s) -
                   1+(1e-6 + x_s+(mx_m0 - x_m)**2)/(1e-6 + mx_s0))
        ...
        if withnoise:
            ...
            if dim == 2:
                ...
                eps = torch.cuda.FloatTensor(
                    sigma.size()[0], sigma.size()[1], 1).normal_(0, 1)
            else:
